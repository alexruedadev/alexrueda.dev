<template>
<div id="liner" class="max-w-7xl mx-auto border-dashed border-l border-r">
  <div class=" text-gray-200">
    <div class="px-4 py-4 max-w-5xl mx-auto sm:px-6 lg:px-8">
      <article data-aos="fade-up">
        <div class="space-y-9">
          <div id="liner" class="py-0.5 border-t border-b border-dashed flex items-center justify-between">
          ...
          <div id="post-body" class="px-4 sm:px-0 lg:px-4 pb-4 rounded-lg">
            <a
              target="_blank"
              rel="noreferrer"
            ...
            >
              <div class="flex flex-items-center justify-center rounded-full">
                <UserAvatar :photoURL="post.author.image" :name="post.author.name" class="w-10 h-10 border-2 border-blue-first hover:border-hot-pink -mt-5 bg-gray-900 rounded-full"/>
              </div>
            </a>
            <header class="py-4">
              <div class="space-y-1 text-center">
                <div id="section-title">
                  <h1 class="text-2xl font-extrabold tracking-tight">{{ post.title }}</h1>
                </div>

                <dl class="">
                  <div><dt class="sr-only">Published on</dt>
                    <dd class="text-xs font-medium text-gray-500">
                      <a target="_blank" rel="noreferrer" :href="`https://twitter.com/${post.author.twitter}`"><span class="text-blue-first hover:text-indigo-300">{{ post.author.name }}</span></a>
                ...
                <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div class="px-3 text-xs flex flex-shrink-0 py-1 mt-1 mr-2 bg-gray-800 rounded-md hover:bg-indigo-700" v-for="(tag, index) in post.tags" :key="`tag-${index}`">{{ tag }}</div>
                </div>
              </div>
            </header>

            <div class="prose break-words my-4 prose-sm max-w-4xl mx-auto">
              <nuxt-content id="post" class="sm:mx-3" :document="post" />
            </div>
          </div>
  ...
  <div @click="scrollToTop" class="cursor-pointer fixed z-50 bottom-4 right-4 w-8 rounded-full bg-gray-900 block text-indigo-600 hover:text-hot-pink">
